...
func (cfg *Configuration) setDerivedDefaults() {
	...
	setDefaultUsersync(cfg.Adapters, openrtb_ext.BidderTurktelekom, "https://sp.programattik.com/s2s_sync?gdpr={{.GDPR}}&gdpr_consent={{.GDPRConsent}}&us_privacy={{.USPrivacy}}&redir="+url.QueryEscape(externalURL)+"%2Fsetuid%3Fbidder%3Dturktelekom%26gdpr%3D{{.GDPR}}%26gdpr_consent%3D{{.GDPRConsent}}%26uid%3D%24%7BUUID%7D")
...
func SetupViper(v *viper.Viper, filename string) {
	...
	v.SetDefault("adapters.turktelekom.endpoint", "http://dev.fargo.iponweb.net/sp_bid")
